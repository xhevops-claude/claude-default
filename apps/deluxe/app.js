/* Deluxe — a second house wireframe, cloned from House Wire (apps/houseplan).
 *
...
async function boot() {
  ...
  const SETTINGS_KEY = 'deluxe-settings';
  let saved = (() => {
    try {
  ...
  for (const b of document.querySelectorAll('#seg-lang .seg-btn')) b.addEventListener('click', () => {
    LANG = b.dataset.lang;
    try { localStorage.setItem('deluxe-lang', LANG); } catch (err) { /* private mode */ }
    applyLang();
  });
  applyLang();
