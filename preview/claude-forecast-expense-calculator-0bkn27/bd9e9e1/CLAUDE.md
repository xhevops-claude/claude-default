...
**The Timeline is the home view** and it opens on the next pay, not on the first
of the month: `build()` marks `model.nextPay` (the earliest pay arrival on or
after today) and `renderTimeline` drops everything before it, totalling the month
it starts mid-way through from what is left ("Rest of Sep"). The simulation
itself still runs from the start of the month — that filter is presentation only,
so the loan and budget bookkeeping behind the opening balance stays whole.
