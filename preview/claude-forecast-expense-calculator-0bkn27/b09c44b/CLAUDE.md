...
| `income.json` | `workday` (either `amount` or `hourlyRate` + `hoursPerDay`, in any currency) plus an `additional` array of extra income |
| `mortgages.json` / `liabilities.json` | debts — same shape, two lists, one engine |
| `budget.json` | fixed monthly budget lines |
| `extras.json` | unplanned one-off expenses, drawn from savings |
| `calendar.json` | per-month work-day `adjustments` (`days: -2`, or `set: 18` for an absolute count) |
...
**The engine.** All debts amortise together in one month-by-month loop, because
they interact: each takes its own instalment (interest = balance × annualRate/12,
the rest is principal), then a pool made of the scenario top-up plus the
instalments of already-cleared debts cascades down the `priority` order. That
rollover — a cleared debt handing its payment to the next one — is the point, so
keep the debts in a single simulation. `priority` is an integer, 1 first, and is
meant to be unique across both lists. A debt whose instalment is below its
...
accumulated across the horizon.
