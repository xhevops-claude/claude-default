...
(function () {
  ...
  var DATA_FILES = ['meta', 'income', 'mortgages', 'liabilities', 'budget', 'extras', 'calendar'];
  ...
  // Mortgages and other liabilities amortise identically; only the label and
  ...
  function collectDebts() {
    ...
    take(data.mortgages.mortgages, 'mortgage');
  ...
  function renderOverview() {
    var rows = model.rows;
    var n = rows.length || 1;
    var avgIncome = model.totals.income / n;
    var avgOut = (model.totals.fixed + model.totals.debt + model.totals.extras) / n;
    var avgSaved = model.totals.saved / n;

    $('hero-saved').textContent = money(avgSaved, { signed: true });
    $('hero-saved').className = 'hero-amount ' + (avgSaved < 0 ? 'is-neg' : 'is-pos');
    $('hero-sub').textContent = 'Average over ' + n + ' months, from ' +
      ymLabel(rows[0].ym, { long: true });
    $('hero-income').textContent = money(avgIncome);
    $('hero-out').textContent = money(avgOut);
    $('hero-rate').textContent = avgIncome > 0
      ? Math.round((avgSaved / avgIncome) * 100) + '%' : '—';
      ...
      return '<div class="card debt-card">' +
        '<div class="debt-head">' +
          ...
          '<span class="debt-id">' +
            ...
            '<span class="debt-meta">' + esc(d.kind === 'mortgage' ? 'Mortgage' : 'Liability') +
              (d.party ? ' · ' + esc(d.party) : '') +
  ...
  function renderLedger() {
    ...
    var debtItems = (data.mortgages.mortgages || []).map(function (m) {
      return { item: m, kind: 'Mortgage' };
    ...
    var debtHtml = ledgerSection('Mortgages and liabilities', 'By repayment priority',
  ...
  function renderFoot() {
    var samples = 0;
    [data.income.additional, data.mortgages.mortgages, data.liabilities.liabilities,
