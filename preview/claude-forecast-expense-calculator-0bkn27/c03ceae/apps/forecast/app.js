...
(function () {
  ...
  function build() {
    ...
    for (var i = 0; i < MAX_MONTHS; i++) {
      ...
      (data.budget.budget || []).forEach(function (it) {
        ...
        var day = Math.min(it.chargeDay || 1, daysInMonth(monthDate.getUTCFullYear(),
          monthDate.getUTCMonth()));
        events.push({
          date: mkDate(monthDate.getUTCFullYear(), monthDate.getUTCMonth(), day),
          kind: 'budget', label: it.label, detail: it.note || '',
  ...
  function renderTimeline() {
    $('timeline-list').innerHTML = model.rows.map(function (r, i) {
      ...
      return '<details class="mrow"' + (i < 2 ? ' open' : '') + '>' +
        '<summary class="mrow-head">' +
          '<span class="mrow-month">' + esc(ymLabel(r.ym, { long: true })) +
            '<span class="mrow-days">' + esc(money(r.income)) + ' in · ' +
            esc(money(r.loan + r.budget + r.extras)) + ' out</span></span>' +
  ...
  function renderLedger() {
    ...
    var budgetHtml = ledgerSection('Fixed monthly budget', money(budgetTotal) + ' / month',
      data.budget.budget || [], function (it) {
        return ledgerRow(it, nativeMoney(it.amount, it.currency),
          'charged on day ' + (it.chargeDay || 1) + (it.note ? ' · ' + it.note : ''));
      });
